    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-md py-2 md:py-3' : 'bg-black py-3 md:py-5'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex space-x-8">
          <a href="/" className={`${isScrolled ? 'text-gialoma-lightgold' : 'text-white'} hover:text-gialoma-gold transition-colors font-medium`}>Home</a>
          <a href="/#about" className={`${isScrolled ? 'text-gialoma-lightgold' : 'text-white'} hover:text-gialoma-gold transition-colors font-medium`}>About</a>
          <a href="/#solutions" className={`${isScrolled ? 'text-gialoma-lightgold' : 'text-white'} hover:text-gialoma-gold transition-colors font-medium`}>Solutions</a>
          <a href="/#services" className={`${isScrolled ? 'text-gialoma-lightgold' : 'text-white'} hover:text-gialoma-gold transition-colors font-medium`}>Services</a>
          <a href="/#contact" className={`${isScrolled ? 'text-gialoma-lightgold' : 'text-white'} hover:text-gialoma-gold transition-colors font-medium`}>Contact</a>
        </nav>

        <div className="flex items-center gap-2">
          {/* Desktop Language Selector - compact size to match login button */}
          <div className="hidden md:block">
            <LanguageSelector isCompact={true} customColor={isScrolled ? undefined : 'white'} />
          </div>
